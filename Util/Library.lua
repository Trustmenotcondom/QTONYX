				...
				function Funcs:addDropdown(title, default, options, callback)
					...
					CreateInstance("UIGradient", {
						Color = ColorSequence.new{
							...
							ColorSequenceKeypoint.new(0.3, Color3.fromRGB(36, 5, 75)),
							ColorSequenceKeypoint.new(0.5, Color3.fromRGB(36, 5, 75)),
							ColorSequenceKeypoint.new(0.7, Color3.fromRGB(36, 5, 75)),
					...
					function ResetDropFunc:Refresh(Newlist)
						...
						if not Found then
							if type(OriginalDefault) == "number" then
								NewIndex = math.clamp(OriginalDefault, 1, math.max(1, #options))
							elseif type(OriginalDefault) == "string" then
								for i, v in ipairs(Newlist) do
									if v == OriginalDefault then
										NewIndex = i
										Found = true
										break
									end
